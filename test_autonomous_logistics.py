...
class AutonomousLogisticsEvidenceTests(unittest.TestCase):
    ...
    def test_aurora_driverless_route_uses_q2_2026_primary_source_geography(self):
        ...
        truck = next(
            row
            for row in self.registry["trucking_operators"]
            if row["operator_id"] == "aurora"
        )
        event = next(
            row
            for row in self.registry["operation_events"]
            if row["event_id"] == "aurora-current-2026"
        )
    ...
    def test_december_2024_atlas_launch_and_january_2025_snapshot_match_primary_source(self):
        ...
        self.assertIn(
            "completed the delivery of 100 loads of proppant",
            source["required_markers"],
        )
        self.assertIn(
            "driverless service with these trucks commenced on December 18th, 2024",
            source["required_markers"],
        )
